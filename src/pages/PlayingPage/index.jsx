	...
	const listLeader = (() => {
		...
		players.forEach((player) => {
			items.push({
				id: player.id,
				name: player.name,
				total: player.scores.reduce((accumulator, currentValue) => accumulator + currentValue, 0),
			});
		});
		...
		>
			...
			<List sx={{ padding: '1rem', pt: 'calc(56px + 1rem)' }}>
				{listLeader.map((player, index) => (
					...
					>
						<Typography sx={{ mr: '1rem', fontWeight: 'bold' }}>Top {index + 1}:</Typography>
						<ListItemAvatar>
	...
	return (
		...
		>
			...
			<Stack alignItems="center" direction="row" gap="0.5rem">
				...
				{match.isFinished && (
					<Button variant="outlined" onClick={onShowResult} startIcon={<SportsScoreRoundedIcon />}>
						<Typography
							sx={{
								fontSize: '13px',
								fontWeight: 'bold',
				...
				<Stack direction="row" gap="0.25rem">
					<Typography
						sx={{
							fontWeight: 'bold',
							fontSize: '14px',
							color: total >= 0 ? (total === 0 ? '#1976D2' : '#008000') : '#D32F2F',
						}}
					>
						Điểm: {total}
					</Typography>
					{currentGameNumber !== 1 && !isFinished && (
						...
						>
							...
							{increasingTrendValue < 0 && (
								<ArrowDownwardSharpIcon sx={{ height: '16px', marginLeft: '-4px' }} />
							)}
			...
			>
				{players.length === 0 && (
					...
					>
						...
						Bạn chưa bất kỳ người chơi nào. <br /> Vui lòng tạo bằng cách ấn nút bên dưới
					</Typography>
				)}
				{players.map((player) => (
					<Player
...
function PlayingPage() {
	...
	const [currentGameNumber, setCurrentGameNumber] = useState(db.getCurrentGameNumber(id));

	const handleContinuePlay = () => {
		if (players?.length < MIN_PLAYERS_OF_MATCH) {
			toast.error(`Số lượng người chơi phải ít nhất ${MIN_PLAYERS_OF_MATCH} người`);
			return;
		}

		const isValid = db.calculateTotalScoreValid(match.id, db.getCurrentGameNumber(id));
	...
	const handleFinishMatch = () => {
		...
		if (!isValidTotal) {
			toast.error(`Tổng số điểm của ván ${gameNumbersInvalid.join(', ')} phải bằng 0`);
	...
	return (
		...
		>
			...
			<LeaderBoard players={players} isOpen={isShowLeaderBoard} onClose={handleLeaderBoardClose} />
		</Box>
	);
}
